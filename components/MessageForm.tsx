...
export default function MessageForm({ proName, proEmail }: { proName: string, proEmail: string }) {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          message, projectType: 'General Message', turnstileToken,
        }),
      })
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="btn-outline font-display tracking-widest">
        Message
      </button>
    )
  }
  ...
  if (success) {
    return (
      <div className="card p-6 text-center">
        ...
        <p className="text-muted text-sm">{proName} will receive your message and reply to your email.</p>
    ...
    <div className="card p-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          ...
          <textarea className="input resize-none min-h-[100px]" placeholder="Write your message..." value={message} onChange={e => setMessage(e.target.value)} />
        </div>
