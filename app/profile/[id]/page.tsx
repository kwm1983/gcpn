...
export default async function ProfilePage({ params }: { params: { id: string } }) {
  ...
  const schemaData = {
    ...
    address: { '@type': 'PostalAddress', addressLocality: pro.city, addressRegion: 'Gulf Coast' },
    url: `https://gulfcoastproductionnetwork.com/profile/${pro.id}`,
    image: pro.avatar_url || undefined,
    description: pro.bio || undefined,
    knowsAbout: pro.skills || [],
  }
  ...
  return (
    <>
      ...
      <div className="pt-16 min-h-screen">
        {/* Cover */}
        <div className="h-48 bg-gradient-to-br from-navy2 to-card flex items-center justify-center text-8xl opacity-40" role="img" aria-label={`${pro.specialty || 'Media'} professional cover`}>🎬</div>
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-end -mt-12 mb-8">
            ...
            <div className="w-24 h-24 rounded-full border-4 border-navy bg-card flex items-center justify-center text-4xl flex-shrink-0">
              {pro.avatar_url
                ? <img src={pro.avatar_url} alt={`${name} - ${pro.specialty} in ${pro.city}`} className="w-full h-full rounded-full object-cover" />
                : '👤'}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-16">
            ...
            <div className="md:col-span-2 flex flex-col gap-6">
                ...
                <div className="card p-6">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {portfolio.map((item: any) => (
                      ...
                      >
                        {item.thumbnail
                          ? <img src={item.thumbnail} alt={item.title || 'Portfolio item'} className="w-full h-full object-cover" />
                          : '🎬'}
                      </a>
                    ))}
                  </div>
              ...
              <div className="card p-6">
                    ...
                    {reviews.map((r: any) => (
                      <div key={r.id} className="bg-navy/40 rounded-xl p-4 border border-white/[0.04]">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="text-gold text-sm" aria-label={`${r.rating} out of 5 stars`}>{'★'.repeat(r.rating)}{'☆'.repeat(5 - r.rating)}</span>
                        </div>
                ...
                <ReviewForm proId={pro.id} proName={firstName} />
              </div>
            </div>
